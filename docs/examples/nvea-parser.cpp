...
public:
    ...
    size_t Read(ICommProtocol::ChannelId channelId, std::span<uint8_t> buffer) override
    {
        ...
        const size_t bytesToRead = std::min(buffer.size(), m_rxBuffer.size());
        std::copy(m_rxBuffer.begin(), m_rxBuffer.begin() + bytesToRead, buffer.begin());
        m_rxBuffer.clear();
        return bytesToRead;
    }
    ...
    {
        if (!this->IsRunning(channelId)) return;
...
int main()
{
    ...
    auto rxMessages = stream.Read<GPGGAMessage>(ch, 10, std::chrono::seconds(1));
